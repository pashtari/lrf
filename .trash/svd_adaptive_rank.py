...
#     p, q = patch_size
#     patches = rearrange(x, "c (h p) (w q) -> (c h w) p q", p=p, q=q)
#     return patches
...
#     p, q = patch_size
#     patches = rearrange(x, "(c h w) p q -> c (h p) (w q)", h=size[0] // p, w=size[1] // q)
#     return patches
...
def svd_encode(
    ...
    if color_space == "RGB":
        ...
        # ###### x: (B, P, P), u: (B, P, P), , s: (B, P), v: (B, P, P) ######
        # total_energy = torch.sum(s**2, dim=-1, keepdim=True)
        # cumulative_energy = torch.cumsum(s**2, dim=-1)
        # threshold_energy = (quality / 100) * total_energy
        # rank = torch.sum(cumulative_energy < threshold_energy, dim=-1) + 1

        # mask = torch.arange(s.shape[-1]).view(1, -1).to(x.device) < rank.unsqueeze(1)
        # s = mask * s
        # ######
        ...
        # ######
        # u = rearrange(u, "b r s -> (b r) s")
        # v = rearrange(v, "b r s -> (b r) s")
        # ######
...
def svd_decode(encoded_image: bytes) -> Tensor:
    ...
    if metadata["color space"] == "RGB":
        ...
        # ######
        # u = rearrange(u, "(b r) s -> b r s", r=min(*metadata["patch size"]))
        # v = rearrange(v, "(b r) s -> b r s", r=min(*metadata["patch size"]))
        # ######
