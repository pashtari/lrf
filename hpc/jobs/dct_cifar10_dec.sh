...
for size in 12 16 20 24 28 32; do
    python eval.py dist.backend=nccl dist.nproc_per_node=2 dist.nnodes=1 task_name=eval_${TASK_NAME} data=cifar10 metric=cifar10 model=dct_model_cifar model.num_classes=10 model.domain=decompressed model.new_size=$size model.pad=false
...
# Evals
for size in 12 16 20 24 28 32; do
    python eval.py dist.backend=nccl dist.nproc_per_node=2 dist.nnodes=1 task_name=eval_${TASK_NAME} data=cifar10 metric=cifar10 model=dct_model_cifar model.num_classes=10 model.domain=decompressed model.new_size=$size model.pad=true
    echo "new_size=$size done."
done
