...
TASK_NAME=interpolate_cifar10_pretrained_resnet50_low
...
for size in $(seq 2 2 36); do
    python eval.py dist.backend=null dist.nproc_per_node=null dist.nnodes=null task_name=eval_${TASK_NAME} data=cifar10 metric=cifar model=interpolate_model_cifar +model.net.pretrained_weights_path=${CKPT_PATH} +model.spatial_dims=2 +model.conv1_kernel_size=3 +model.conv1_stride=1 +model.no_maxpool=true model.num_classes=10 model.rescale=false model.new_size=$size
