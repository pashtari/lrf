...
TASK_NAME=interpolate_tinyimagenet_dec

# Train
python train.py dist.backend=nccl dist.nproc_per_node=2 dist.nnodes=1 task_name=train_${TASK_NAME} data=tinyimagenet metric=imagenet model=interpolate_model_imagenet model.num_classes=200 model.rescale=true

CKPT_PATH=$(cat ${ROOT_DIR}/.temp/train_${TASK_NAME}.txt)


# Evals 
for size in $(seq 2 1 72); do
