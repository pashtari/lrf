...
for size in $(seq 12 4 32); do
    python eval.py dist.backend=nccl dist.nproc_per_node=2 dist.nnodes=1 task_name=eval_${TASK_NAME} data=cifar10 metric=cifar model=dct_model_cifar model.num_classes=10 model.domain=compressed model.new_size=$size
