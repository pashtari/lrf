...
from lrf.factorization import QMF
...
def qmf_rank(size: tuple[int, int], com_ratio: float) -> int:
    """Calculate the rank for QMF based on the compression ratio.
...
def qmf_encode(
    ...
    **kwargs,
    ...
    """
    QMF compression of an image.

    Args:
        image (torch.Tensor): The input image tensor.
        rank (int or tuple[int, int, int], optional): The rank for QMF (default: None).
        quality (float or tuple[float, float, float], optional): The quality for QMF (default: None).
        ...
        bounds (tuple[float, float], optional): The bounds for QMF (default: (-16, 15)).
        dtype (torch.dtype, optional): The data type for encoding (default: torch.int8).
        **kwargs: Additional arguments for QMF decomposition.
    ...
    if color_space == "RGB":
        if patch:
            ...
            qmf = QMF(rank=R, bounds=bounds, factor=(0, 1), **kwargs)
            u, v, _ = qmf.decompose(x.unsqueeze(0))
        ...
        if patch:
            ...
            for i, channel in enumerate((y, cb, cr)):
                ...
                qmf = QMF(rank=R, bounds=bounds, factor=(0, 1), **kwargs)
                u, v, _ = qmf.decompose(channel.unsqueeze(0))
                u, v = u.squeeze(0), v.squeeze(0)
...
def qmf_decode(encoded_image: bytes) -> torch.Tensor:
    """
    Decode an QMF-compressed image.
