...
# class PatchSVDInterpolate(PatchSVD):
#     # """Implements interpolattion based on PatchSVD."""
...
#         x: Tensor,
#         compression_ratio: Optional[float] = None,
#         # new_grid_size: Optional[Tuple[int, int]] = None,
#         new_patch_size: Optional[Tuple[int, int]] = None,
#         return_factors: bool = True,
#         **kwargs
#     ) -> Union[Tuple[Tensor, Tensor], Tensor]:

#         # grid_size = [s // p for s, p in zip(x.shape[-2:], self.patch_size)]

#         # if new_grid_size is None:
#         #     new_grid_size = self.get_new_size(grid_size, compression_ratio)

#         if new_patch_size is None:
#             new_patch_size = self.get_new_size(self.patch_size, compression_ratio)
...
#         u, v = self.depatchify_uv(x, u, v)

#         # u = F.interpolate(u, new_grid_size, **kwargs)
#         v = F.interpolate(v, new_patch_size, **kwargs)

#         u = rearrange(u, "(b r) 1 h w -> b r 1 h w", r=rank)
#         v = rearrange(v, "(b r) c p1 p2 -> b r c p1 p2", r=rank)

#         # self.real_compression_ratio = self.get_compression_ratio(grid_size, new_grid_size)
#         self.real_compression_ratio = self.get_compression_ratio(
...
#             return u, v
#         else:
#             y = torch.einsum("b r i h w, b r c p1 p2 -> b i h w c p1 p2", u, v)
#             y = rearrange(y, "b 1 h w c p1 p2 -> b c (h p1) (w p2)")
#             return y

#     def decompress(
#         self,
#         x: Union[Tuple[Tensor, Tensor], Tensor],
#         size: Tuple[int, int] = None,
#         **kwargs
#     ) -> Tensor:

#         if isinstance(x, Tensor):
#             y = F.interpolate(x, size, **kwargs)
#             return y
#         else:
#             u, v = x
#             batch_size = u.shape[0]

#             u = rearrange(u, "b r 1 h w -> (b r) 1 h w")
#             v = rearrange(v, "b r c p1 p2 -> (b r) c p1 p2")

#             # grid_size = [s // p for s, p in zip(size, self.patch_size)]
#             # u = F.interpolate(u, grid_size)
#             v = F.interpolate(v, self.patch_size)

#             u = rearrange(u, "(b r) 1 h w -> b r 1 h w", b=batch_size)
#             v = rearrange(v, "(b r) c p1 p2 -> b r c p1 p2", b=batch_size)

#             y = torch.einsum("b r i h w, b r c p q -> b h w c p q", u, v)
#             y = rearrange(y, "b h w c p1 p2 -> b c (h p1) (w p2)")
#             return y

#     def forward(self, x: Tensor, *args, **kwargs) -> Tensor:
#         compressed_image = self.compress(x, *args, **kwargs)
#         decompressed_image = self.decompress(compressed_image, x.shape[-2:])
#         return decompressed_image
