...
def compress_svd_patches_v1(image, compression_ratio, patch_size=8):
...
def compress_svd_patches_v2(image, compression_ratio, patch_size=8):
    """Compress the image using SVD on patches."""
    image = torch.tensor(image, dtype=torch.float32).permute(-1, 0, 1)
...
for ratio in compression_ratios:
    ...
    # SVD Patches V1
    compressed_svd_patches_v1 = compress_svd_patches_v1(image, ratio)
    err_values["SVD Patches V1"].append(mae(image, compressed_svd_patches_v1))
    compressed_images["SVD Patches V1"].append(compressed_svd_patches_v1)

    # SVD Patches V2
    compressed_svd_patches_v2 = compress_svd_patches_v2(image, ratio)
    err_values["SVD Patches V2"].append(mae(image, compressed_svd_patches_v2))
    compressed_images["SVD Patches V2"].append(compressed_svd_patches_v2)
