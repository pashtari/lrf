...
experiment_name = 'imagenet_0_1'
data_dir = "/scratch/leuven/362/vsc36284"
...
dataset = ImageNet(root=data_dir, split="val", transform=transforms)
nb_samples = len(dataset)
# draw subset_ratio shuffled indices 
indices = torch.randperm(nb_samples)[:10]
...
def calc_compression_metrics(image):
    ...
    # IMF - YCbCr
    for quality in np.linspace(0, 40, 50):
        enocoded = lrf.imf_encode(
            image,
            color_space="YCbCr",
...
for image_id, (image, img_lbl) in enumerate(dataloader):
    print(f"processing image {image_id + 1}/{len(dataloader)}", flush=True)
    calc_compression_metrics(image=image.squeeze())


image_num = len(dataloader)
# Plotting the results: PSNR vs bpp
plot_result(x_values=bpps, y_values=psnr_values, x_axis_fixed_values=np.linspace(0, 1, 50))
