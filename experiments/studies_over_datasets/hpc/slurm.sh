...
sbatch --account=lp_inspiremed --job-name=cmp_imgnet --clusters=genius --nodes=1 --ntasks-per-node=2 --cpus-per-task=9 --time=24:00:00 comparison_imagenet.sh

# Comparison of classification performance over a subset of Imagenet
sbatch --account=lp_inspiremed --job-name=clsi_IMF --clusters=genius --nodes=1 --ntasks-per-node=1 --cpus-per-task=9 --gres=gpu:1 --time=24:00:00 --partition=gpu_p100 IMF_imagenet_classification.sh
sbatch --account=lp_inspiremed --job-name=clsi_SVD --clusters=genius --nodes=1 --ntasks-per-node=1 --cpus-per-task=9 --gres=gpu:1 --time=24:00:00 --partition=gpu_p100 SVD_imagenet_classification.sh
sbatch --account=lp_inspiremed --job-name=clsi_JPG --clusters=genius --nodes=1 --ntasks-per-node=1 --cpus-per-task=9 --gres=gpu:1 --time=24:00:00 --partition=gpu_p100 JPEG_imagenet_classification.sh